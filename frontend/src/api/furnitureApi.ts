 ...
 */
export async function saveFurnitureForFloor(floorId: number, items: FurnitureItem[]) {
    // 👇 This is what you'd send to backend (for development/debug)
    const payload = {
        floorId,
        furniture: items
    };
    console.log('[Furniture API] Would send to backend:', JSON.stringify(payload, null, 2));
    // 👆
