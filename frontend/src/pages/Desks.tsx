...
import { getFloorData, getFloors, saveFurnitureForFloor } from '../api/furnitureApi';
import {
    Box,
    FormControl,
    InputLabel,
...
const Desks = () => {
    ...
    const handleFloorChange = async (e: any) => {
        ...
        if (selectedFloor !== null) {
            const changed = JSON.stringify(items) !== JSON.stringify(lastSavedItems.current);
            if (changed) {
    ...
    return (
        <Box sx={{ width: '100%', py: 4 }}>
            <Paper elevation={2} sx={{ p: 3, maxWidth: CANVAS_MAX_WIDTH + 64, mx: 'auto', mb: 3 }}>
                <Box
                    display="flex"
                    alignItems="center"
                    gap={3}
